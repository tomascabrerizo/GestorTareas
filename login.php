...
//Si el usuario creo un proyecto
if(isset($_POST['crearP'])){
    $nombre = $_POST["nombre"];

    if($nombre != ""){
        $usuario->nuevoProyecto($nombre);
        header("location:$_SERVER[PHP_SELF]");
    }
}

//Si el usuario creo una tarea
if(isset($_POST['crearT'])){
    $proyectoID = $_POST["selecProyecto"];
    $nombre = $_POST["nombre"];
    $descripcion = $_POST["descripcion"];

    if($nombre != "" && $descripcion != ""){
        $usuario->nuevaTarea($proyectoID, $nombre, $descripcion, 0);
        header("location:$_SERVER[PHP_SELF]");
    }
}
